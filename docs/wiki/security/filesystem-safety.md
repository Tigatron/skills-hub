...
The cleanup audit extended exact ownership proof to lifecycle paths outside the generic executor. Relocation staging and old-Vault cleanup bind reviewed device/inode identity, Vault ID, operation-derived exact path, and marker evidence. Old-Vault cleanup atomically quarantines the reviewed source before recursive removal and restores it if post-rename identity changed. GC recovery reconstructs the sole operation/digest-derived pending-delete path and rejects copied ownership evidence elsewhere. Capability probes, journal/object staging, and durable-write temporary paths quarantine only the captured application-created identity; replacement content is preserved. Product child-process and failpoint matrices exercise takeover, deploy, undeploy, Trash, restore, and relocation recovery without widening cleanup scope.

Local diagnostics hold a no-follow descriptor for their application-support root, operate on validated segment basenames relative to that descriptor, derive retention age from service-created names rather than mutable timestamps, and stop on foreign entries or root/segment identity changes. Export treats persisted records as untrusted and reconstructs a strict redacted schema before preview or save. Saving uses a no-follow destination-parent descriptor, exclusive temporary creation, no-replace publication, and digest-bound prepared bytes. These logs are local-only and separate from Activity and recovery evidence.
